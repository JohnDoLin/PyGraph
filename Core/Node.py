...
class Node:
    ...
    def draw_node(self, window: str, scale: float = 1, offset: Vec2 = Vec2([0, 0])):
        ...
        if self.created:
            ...
            dpg.draw_circle(pos_array, self.style["radius"] * scale, tag=self.uuid, parent= window, fill=self.style["color"], thickness=self.style["border_width"]*scale, color=self.style["border_color"])
            ...
            dpg.draw_circle(pos_array, self.style["radius"] * scale, tag=self.uuid, parent= window, fill=self.style["color"], thickness=self.style["border_width"] * scale, color=self.style["border_color"])
            dpg.draw_text(text_array, self.text, tag=str(self.uuid)+'text', parent=window, size=self.style["radius"] * scale)
            self.created = True
